// bolsa-empleo-fe/src/features/registro/pages/RegistroOferentePage.jsx
import { useState } from 'react'
import { useNavigate, Link } from 'react-router-dom'
import { registroService } from '../services/registroService'
import Alert from '../../../shared/components/Alert'

const VACIO = { correo: '', identificacion: '', password: '', nombre: '', apellido: '', nacionalidad: '', telefono: '', correoOferente: '', residencia: '' }

export default function RegistroOferentePage() {
    ...
    async function handleSubmit(e) {
        ...
        setCargando(true)
        try {
            await registroService.registrarOferente(form)
    ...
    return (
        <div style={{ maxWidth: 550, margin: '2rem auto' }}>
            <h2>Registrar oferente</h2>
            <Alert tipo={mensaje.tipo} mensaje={mensaje.texto} />
            <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                <fieldset style={fieldsetStyle}>
                    <legend>Cuenta de acceso</legend>
                    {[['correo','Correo','email'],['identificacion','Cédula','text'],['password','Contraseña','password']].map(([n,l,t]) => (
                ...
                <fieldset style={fieldsetStyle}>
                    ...
                    {[['nombre','Nombre','text'],['apellido','Apellido','text'],['nacionalidad','Nacionalidad','text'],['telefono','Teléfono','text'],['correoOferente','Correo público','email'],['residencia','Residencia','text']].map(([n,l,t]) => (
                        <label key={n} style={{ display:'block', marginBottom:'0.5rem' }}>{l}
